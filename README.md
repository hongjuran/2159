...
- `CLIENT_ID`: [개발자 포털](https://discord.com/developers/applications) > 만든 애플리케이션 선택 > **General Information** > `Application ID`
- `GUILD_ID`: 디스코드 앱 **설정 > 고급**에서 **개발자 모드**를 켠 뒤, 봇을 사용할 서버 아이콘을 우클릭 > **ID 복사**

## 슬래시(`/`) 명령어 등록

디스코드에서 `/`를 입력했을 때 이 봇의 명령어 목록이 자동으로 뜨게 하려면, `.env` 설정 후 아래 명령을 **한 번만** 실행하세요. (명령어를 추가/수정할 때마다 다시 실행하면 됩니다.)
...
음성 채널에 먼저 접속한 상태에서 명령어를 입력합니다. `/`로 시작하는 **슬래시 명령어**를 입력하면 디스코드가 자동으로 명령어 목록/설명을 보여줍니다. 기존 `!` 접두사 명령어도 그대로 사용할 수 있습니다.

| 슬래시 명령어 | `!` 명령어 | 설명 |
| --- | --- | --- |
| `/play url:<URL>` | `!play <URL>` | 유튜브/사운드클라우드 URL 재생 (대기열에 추가) |
| `/skip` | `!skip` | 다음 곡으로 넘기기 |
| `/stop` | `!stop` | 재생 정지 및 대기열 비우기 |
| `/pause` / `/resume` | `!pause` / `!resume` | 일시정지 / 다시재생 |
| `/queue` | `!queue` | 대기열 보기 (곡 목록 + 조작 버튼) |
| `/nowplaying` | `!nowplaying` | 현재 재생 곡 정보 (진행 바 + 조작 버튼) |
| `/volume value:<0-100>` | `!volume <0-100>` | 볼륨 조절 |
| `/shuffle` | `!shuffle` | 대기열 섞기 |
| `/leave` | `!leave` | 음성 채널 나가기 |
| `/help` | `!help` | 명령어 목록 보기 |

명령어 접두사는 `.env`의 `COMMAND_PREFIX`로 바꿀 수 있습니다.

곡이 재생되면 앨범 아트, 진행 바, 요청자 정보가 담긴 카드가 뜨고 그 아래 ⏸️ ⏭️ ⏹️ 🔀 📜 버튼으로 직접 조작할 수 있습니다.
